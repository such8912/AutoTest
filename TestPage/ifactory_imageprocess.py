...
class ImageProcessPage(BasePage):
    # 定位器，通过元素属性定位元素对象
    yun_guang_yun_se_loc = ('XPATH', '//label[@title="GF1遥感卫星影像匀光匀色流程"]')
    dom_loc = ('XPATH', '//label[@title="GF1遥感卫星影像DOM生成流程"]')
    xiang_qian_fen_fu_loc = ('XPATH', '//label[@title="GF1遥感卫星影像真彩色融合及镶嵌分幅流程"]')
    ...
    def click_gf1_ygys(self):
        elem = self.find_element(self.yun_guang_yun_se_loc)
    ...
    # 双击GF1遥感卫星影像镶嵌分幅流程
    def click_gf1_xqff(self):
        elem = self.find_element(self.xiang_qian_fen_fu_loc)
        ActionChains(self.driver).double_click(elem).perform()
